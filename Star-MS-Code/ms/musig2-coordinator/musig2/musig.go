...
// 产一个签名者的部分签名s_i
func SignMulti(msg []byte, key *Key, nonces []*Key, R kyber.Point, RValues []kyber.Point, X kyber.Point, L []byte) kyber.Scalar {
	c := computeC(X, R, msg)
	...
	for j, n := range nonces {
		b := computeB(uint64(j), msg, RValues, X)
...
// 计算承诺值
func ComputeR(msg []byte, RValues []kyber.Point, X kyber.Point) kyber.Point {
	R := curve.Point().Null()
...
// 计算承诺值系数b_j
func computeB(j uint64, msg []byte, nonces []kyber.Point, X kyber.Point) kyber.Scalar {
	if j == 0 {
